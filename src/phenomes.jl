"""
    plot(type::Type{T}, phenomes::Phenomes)::T where {T<:DistributionPlots}
...
x = plot(DistributionPlots, phenomes)
fnames = saveplots(x)
rm.(fnames)
```
"""
function plot(type::Type{T}, phenomes::Phenomes)::T where {T<:DistributionPlots}
    # type = DistributionPlots; phenomes = Phenomes(n=10, t=3); phenomes.entries = string.("entry_", 1:10); phenomes.populations = sample(["pop_1", "pop_2"], 10, replace=true); phenomes.traits = ["A", "B", "C"]; phenomes.phenotypes = rand(10,3); phenomes.phenotypes[1, 1] = missing;
    ...
    i::Int64 = 0
    # Per trait
    fig_per_trait = CairoMakie.Figure();
    axs = CairoMakie.Axis(fig_per_trait[1,1], title="test")
    for (i, trait) in enumerate(phenomes.traits)
        # i = 1; trait = phenomes.traits[i]
        idx = findall(.!ismissing.(phenomes.phenotypes[:, i]) .&& .!isnan.(phenomes.phenotypes[:, i]) .&& .!isinf.(phenomes.phenotypes[:, i]))
        y = phenomes.phenotypes[idx, i]
        CairoMakie.density!(axs, y, offset = -i/4, color = (:slategray, 0.4), bandwidth = 0.1)
    end
    plots[1] = fig_per_trait
...
"""
    plot(type::Type{T}, phenomes::Phenomes)::T where {T<:ViolinPlots}
...
x = plot(ViolinPlots, phenomes)
fnames = saveplots(x)
rm.(fnames)
```
"""
function plot(type::Type{T}, phenomes::Phenomes)::T where {T<:ViolinPlots}
    ...
    # Instantiate the x-axis values, i.e. categories for violin plotting
    df_reshaped.x_axis_value .= 1
    # Add the size of each category and extract their respective x-axis value
    lengths = combine(groupby(df_reshaped[idx, :], :variable), :value => length)
    for (i, x) in enumerate(eachrow(lengths))
        # i = 1; x = eachrow(lengths)[i]
        idx = findall(df_reshaped.variable .== x[1])
        df_reshaped.variable[idx] .= string(x[1], "\n(n=", x[2], ")")
        df_reshaped.x_axis_value[idx] .= i
    end
    


    idx = findall(
        .!ismissing.(df_reshaped.value) .&&
        .!isnan.(df_reshaped.value) .&&
        .!isinf.(df_reshaped.value)
    )
    df_reshaped = df_reshaped[idx, :]
    f = CairoMakie.Figure();
    ax = CairoMakie.Axis(f[1,1], xlabel="Trait values", yticks=(1:size(lengths,1), lengths.variable))
    CairoMakie.violin!(
        ax,
        df_reshaped.x_axis_value, 
        df_reshaped.value; 
    ...
    means = combine(groupby(df_reshaped, :x_axis_value), :value => mean)
    CairoMakie.text!(
        ax,
        means.value_mean, 
        means.x_axis_value,
        text=string.(round.(means.value_mean, digits=2)),
        align=(:left, :center)
    )
    f
...
"""
    plot(type::Type{T}, phenomes::Phenomes; color_scheme::Symbol=:YlGn)::T where {T<:CorHeatPlots}
...
```julia
phenomes = Phenomes(n=100, t=3); phenomes.entries = string.("entry_", 1:100); phenomes.populations = StatsBase.sample(string.("pop_", 1:5), 100, replace=true); phenomes.traits = ["A", "B", "C"]; phenomes.phenotypes = rand(Distributions.MvNormal([1,2,3], LinearAlgebra.I), 100)'; phenomes.phenotypes[1, 1] = missing;
x = plot(CorHeatPlots, phenomes, color_scheme=:viridis)
fnames = saveplots(x)
rm.(fnames)
```
"""
function plot(type::Type{T}, phenomes::Phenomes; color_scheme::Symbol = :YlGn)::T where {T<:CorHeatPlots}
...
"""
    plot(type::Type{T}, phenomes::Phenomes; color_scheme::Symbol=:YlGn)::T where {T<:TreePlots}
...
x = plot(TreePlots, phenomes)
fnames = saveplots(x)
rm.(fnames)
```
"""
function plot(type::Type{T}, phenomes::Phenomes; color_scheme::Symbol = :default)::T where {T<:TreePlots}
